...
import { ArrowLeft, Coins, TrendingUp, Calendar, MapPin, FileText, Target, MoreVertical, ChevronLeft } from "lucide-react";
...
export default function DonorProjectAnalytics() {
  ...
  const fetchFundUtilizations = async () => {
    try {
      const token = localStorage.getItem('token');
      const url = `http://localhost:8081/api/fund-utilization/project/${projectId}`;
      console.log('DonorProjectAnalytics: Fetching fund utilizations for project:', projectId);
      console.log('DonorProjectAnalytics: Making request to:', url);
      ...
      console.log('DonorProjectAnalytics: Fund utilization response status:', response.status);
      
      if (response.ok) {
        const data = await response.json();
        console.log('DonorProjectAnalytics: Fund utilization data received:', data);
        setFundUtilizations(data || []);
      } else {
        console.error('Failed to fetch fund utilizations:', response.status, await response.text());
      }
    } catch (error) {
  ...
  const fetchDonorContribution = async () => {
    try {
      const token = localStorage.getItem('token');
      const donorId = getDonorIdFromStorage();
      
      if (!donorId) {
        console.log('DonorProjectAnalytics: No donorId found in localStorage');
        return;
      }
      
      const url = `http://localhost:8081/api/donations/donor/${donorId}/project/${projectId}/total`;
      console.log('DonorProjectAnalytics: Fetching donor contribution from:', url);
      ...
      console.log('DonorProjectAnalytics: Donor contribution response status:', response.status);
      
      if (response.ok) {
        const total = await response.json();
        console.log('DonorProjectAnalytics: Donor contribution data received:', total);
        setDonorContribution(total || 0);
      } else {
        console.error('Failed to fetch donor contribution:', response.status, await response.text());
      }
    } catch (error) {
              ...
              <div className="grid grid-cols-4 gap-6 mt-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex items-baseline gap-2">
                    <span className="text-2xl font-bold">৳{Math.round(donorContribution).toLocaleString()}</span>
                  </div>
